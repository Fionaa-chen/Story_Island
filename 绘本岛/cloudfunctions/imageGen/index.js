  ...
  if (action === 'generateRoundImage') {
    ...
    for (let attempt = 0; attempt < maxRetry; attempt++) {
      try {
        ...
        console.error(`[ImageGen] 第${roundIndex || '?'}轮 第${attempt + 1}次尝试失败${isRateLimit ? '(速率限制)' : ''}:`, errMsg);
        
        if (attempt < maxRetry - 1) {
          // 速率限制用更长的指数退避：8s, 16s；其他错误：3s, 6s
          const baseDelay = isRateLimit ? 8000 : 3000;
          const delay = baseDelay * Math.pow(2, attempt);
          console.log(`[ImageGen] 第${roundIndex || '?'}轮 ${delay / 1000}秒后重试...`);
