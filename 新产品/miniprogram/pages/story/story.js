...
Page({
  ...
  async getAIResponse() {
    ...
    try {
      const response = await this.callAI(this.chatHistory);
  ...
  /**
   * 异步触发某一轮的图片生成（后台运行，不阻塞对话）
   */
  triggerRoundImageGeneration(roundIndex, scenePrompt) {
    const { book, role, scene } = this.data;
    const idx = roundIndex - 1; // 转为数组下标 (0-4)
    
    console.log(`[RoundImage] 第${roundIndex}轮 异步生图开始: ${scenePrompt.substring(0, 50)}...`);
    ...
    // 封装单次调用逻辑（供重试复用）
    const doGenerate = () => {
      return this.callCloudFunctionWithTimeout('imageGen', {
    ...
    const updateImage = (imageUrl) => {
      ...
      // 如果所有图都好了，标记 done
      if (loadedCount >= 5) {
        this.setData({ 'endingGrid.status': 'done' });
      }
    ...
    // 异步调用云函数生成图片（不 await），失败后自动重试一次
    const imagePromise = doGenerate().then((res) => {
      if (res.success && res.imageUrl) {
        return updateImage(res.imageUrl);
      } else {
        console.warn(`[RoundImage] 第${roundIndex}轮 图片生成失败:`, res.error, '，3秒后重试...');
        // 第一次失败，延迟3秒后重试
        return new Promise(resolve => setTimeout(resolve, 3000)).then(() => doGenerate()).then((retryRes) => {
          if (retryRes.success && retryRes.imageUrl) {
            console.log(`[RoundImage] 第${roundIndex}轮 重试成功`);
            return updateImage(retryRes.imageUrl);
          }
          console.warn(`[RoundImage] 第${roundIndex}轮 重试仍失败:`, retryRes.error);
          return null;
        });
      }
    }).catch((err) => {
      console.warn(`[RoundImage] 第${roundIndex}轮 图片生成异常:`, err.message, '，3秒后重试...');
      // 异常时也重试一次
      return new Promise(resolve => setTimeout(resolve, 3000)).then(() => doGenerate()).then((retryRes) => {
        if (retryRes.success && retryRes.imageUrl) {
          console.log(`[RoundImage] 第${roundIndex}轮 重试成功`);
        ...
        console.warn(`[RoundImage] 第${roundIndex}轮 重试仍失败:`, retryRes.error);
        this.setData({ roundImageHint: '' });
        return null;
      }).catch((retryErr) => {
        console.warn(`[RoundImage] 第${roundIndex}轮 重试异常:`, retryErr.message);
        this.setData({ roundImageHint: '' });
        return null;
      });
    });
  ...
  async generateEndingGrid(endingTitle, endingComment) {
    ...
    try {
      ...
      if (missingIndices.length > 0) {
        console.log(`[EndingGrid] 发现 ${missingIndices.length} 张缺失图片，尝试补生: 位置 ${missingIndices.map(i => i + 1).join(', ')}`);
        
        // 对缺失的图片逐张补生（带重试的 generateSingleImage）
  ...
  async generateSingleImage(prompt, index) {
    ...
    for (let attempt = 0; attempt < maxRetry; attempt++) {
      try {
        ...
        console.warn(`[EndingGrid] 图片 ${index + 1} 第 ${attempt + 1} 次失败:`, err.message || err.errMsg);
  ...
  async onSaveGrid() {
    ...
    try {
      ...
      // ====== 2. 绘制经典原文纹理层（亮色低透明度） ======
      const quotes = book.classicQuotes || [];
      if (quotes.length > 0) {
        const texRgb = endingDarkColors._texRgb;
        ctx.save();
        const textureConfigs = [
          { x: -20, y: canvasHeight * 0.08, rotate: -0.21, size: 20 },
          { x: canvasWidth * 0.5, y: canvasHeight * 0.28, rotate: 0.14, size: 16 },
          { x: -10, y: canvasHeight * 0.52, rotate: -0.10, size: 18 },
          { x: canvasWidth * 0.3, y: canvasHeight * 0.75, rotate: 0.17, size: 17 },
          { x: canvasWidth * 0.1, y: canvasHeight * 0.90, rotate: -0.26, size: 15 },
        ];
        for (let i = 0; i < Math.min(quotes.length, textureConfigs.length); i++) {
          const cfg = textureConfigs[i];
          ctx.save();
          ctx.translate(cfg.x, cfg.y);
          ctx.rotate(cfg.rotate);
          ctx.fillStyle = `rgba(${texRgb.r}, ${texRgb.g}, ${texRgb.b}, 0.10)`;
          ctx.font = `300 ${cfg.size}px "PingFang SC", serif`;
          ctx.textAlign = 'left';
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (imgUrl) {
          try {
            ...
            this.roundRect(ctx, pos.x, pos.y, pos.w, pos.h, imgRadius);
            ctx.clip();
            ctx.fillStyle = 'rgba(245, 238, 228, 0.6)';
          ...
          this.roundRect(ctx, pos.x, pos.y, pos.w, pos.h, imgRadius);
          ctx.clip();
          ctx.fillStyle = i < 5 ? 'rgba(245, 238, 228, 0.6)' : 'rgba(245, 238, 228, 0.25)';
          ctx.fillRect(pos.x, pos.y, pos.w, pos.h);
          if (i === 5) {
            // 虚线边框效果
            ctx.restore();
      ...
      if (endingInfo && endingInfo.comment) {
        ctx.fillStyle = 'rgba(180, 160, 140, 0.32)';
  ...
  // 调用AI —— 优先调用 DeepSeek 云函数，失败时降级本地模拟
  callAI(messages) {
    return new Promise((resolve, reject) => {
      // 调用 DeepSeek 云函数（设置60秒超时，DeepSeek API 响应较慢）
      wx.cloud.callFunction({
        name: 'deepseek',
        data: {
          messages: messages,
        },
        timeout: 60000, // 60秒超时，避免默认3秒超时
        success: (res) => {
          if (res.result && res.result.success && res.result.reply) {
            ...
            resolve(this.getLocalResponse());
          }
        },
        fail: (err) => {
          console.warn('[AI] 云函数调用失败，使用本地模拟:', err);
          resolve(this.getLocalResponse());
        }
      });
    });
